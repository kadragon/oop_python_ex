        ...
        for i in notices:
            try:
                if i.select('span.text-info')[0].getText() == '[제출가능]':
                    ...
                    extract = bs(s.get('https://go.sasa.hs.kr' + link).text, 'html.parser')
                    due_date = extract.select('small.text-info')[0].getText().split(' ')[3]
                    date = float(due_date.split('-')[1]+'.'+due_date.split('-')[2])
