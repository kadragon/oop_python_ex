...
with requests.Session() as sess:
    ...
    refresh = True
    while refresh:
        music_soup = bs(sess.get('https://go.sasa.hs.kr/RcmndMusic/musicView').text, 'html.parser')
        table = music_soup.select('table tbody tr')
        print('| 순위 | 좋아요 | '+' '*22+' 곡명 '+' '*22+' | '+' '*19+' 링크 '+' '*18+' |   추천자   | 추천시간 | 좋아요 여부 |')
        idx_list = []
        for i in table:
            ...
            print('| %4s | %6s | %s | %s | %-6s | %s |      %s      |' %(rank, like_number, preformat_cjk(title, 50), link, person, time, 'O' if button_clicked else 'X'))
        ...
        while True:
            ...
            if int(n) == 0:
                ...
                n = int(n)
                idx = idx_list[n-1][0][6:] if idx_list[n-1][1] else idx_list[n-1][0][4:]
                action = 'delete' if idx_list[n-1][1] else 'insert'
                print(idx, action)
                res = sess.post('https://go.sasa.hs.kr/RcmndMusic/likeControl', data={'musicIdx': int(idx), 'action': action})
