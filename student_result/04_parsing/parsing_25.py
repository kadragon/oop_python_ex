...
with requests.Session() as s:
    ...
    day_num = get_DayNum()
    PAGE_TOTAL = {"22":16, "23":7,"16":13,"2":26} # 각 공지별로 페이지 수를 저장한 dictionary 

    board_list = [[] for i in range(day_num+1)] # 게시물이 올라온 날짜 별로 기록해서 저장하는 이중 리스트
    
    for page_num in range(1,PAGE_TOTAL[str(CATEGORY_NUM)]): # 사용자가 선택한 CATEGORY_NUM의 PAGE를 가져와서 그만큼 탐색
        ...
        board_list_data = section_board_list_data.select('div.box-body tbody span.hidden-xs') # 게시물의 제목 가져오기
        url_list_data = section_board_list_data.select('div.box-body tbody a') # 게시물의 url 주소 가져오기
        time_list_data = section_board_list_data.select('div.box-body tbody time') # 게시물이 올라온 날짜 가져오기

        url_list = [] # url 주소를 저장해두는 list
        for data in url_list_data: 
            if 'board'== data.get('href').split('/')[1]: # 예시로 /board/lists/page/2의 형식을 가진 데이터만 골라서 저장하기
                url = "https://go.sasa.hs.kr"+'/'.join(data.get('href').split('/')) # https://go.sasa.hs.kr/를 포함한 주소를 담은 변수
                url_list.append(url) # url_list에 추가해준다
                

        for i in range(len(time_list_data)): # 데이터들은 게시물이 올라온 순서대로 저장되므로 3개의 list에 있는 요소는 동일한 것을 가리킨다
            ...
            index = -(list_time - datetime.datetime.now()).days # 오늘로부터 며칠 전에 올라온 게시물인지 확인하기 위한 과정
            
            if index <= day_num: # 사용자가 보려고 하는 기한 안에 들어와있는 게시물이라면
                dataset = (board_list_data[i].text, url_list[i]) # (제목, url주소)를 묶어서 저장하는 tuple 변수
                board_list[index].append(dataset) # 위의 dataset을 board_list[며칠 전의 게시물인지]에 추가해준다 예시로 2일전의 게시물이면 board_list[2]에 저장

    daysago = 0
    for board in board_list: 
        cnt_idx = 0
        print(("="*11+" %02d days ago " +"="*11) % daysago) # 며칠 전의 게시물인지 제목을 출력
        for data in board:
            print("[%d] 제목:%s url:%s" % (cnt_idx, data[0], data[1])) # [출력순서] 제목:제목 url:url 의 형식으로 출력
        if len(board)==0:
