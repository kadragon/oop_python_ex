...
def Make_chart(soup):  #[' 순위', 제목' , '부른사람']
    N_chart_name = soup.select('div._tracklist_mytrack tbody tr td a.title span.ellipsis')
    N_chart_artist = soup.select('div._tracklist_mytrack tbody tr td._artist a ')
...
#차트들 정리 (Top 50)
Naver_chart_1 = get_html('https://music.naver.com/listen/top100.nhn?domain=DOMESTIC&page=1')   #네이버 음악차트 
Naver_soup_1 = bs4.BeautifulSoup(Naver_chart_1, 'html.parser')
N_Music_chart_part_1 = Make_chart(Naver_soup_1)

Naver_chart_2 = get_html('https://music.naver.com/listen/top100.nhn?domain=DOMESTIC&page=2')   #네이버 음악차트 
...
while True:
    ...
    if command =='차트':
        ...
        if gerne =='발라드':
            Naver_chart_ballade = get_html('https://music.naver.com/listen/genre/top100.nhn?domain=DOMESTIC&genre=K01')   #네이버 발라드 음악차트 
            Naver_soup_ballade = bs4.BeautifulSoup(Naver_chart_ballade, 'html.parser')
            ...
            Naver_chart_dance = get_html('https://music.naver.com/listen/genre/top100.nhn?domain=DOMESTIC&genre=K02')   #네이버 발라드 음악차트 
            Naver_soup_dance = bs4.BeautifulSoup(Naver_chart_dance, 'html.parser')
            ...
            Naver_chart_hiphop = get_html('https://music.naver.com/listen/genre/top100.nhn?domain=DOMESTIC&genre=K03')   #네이버 발라드 음악차트 
            Naver_soup_hiphop = bs4.BeautifulSoup(Naver_chart_hiphop, 'html.parser')
