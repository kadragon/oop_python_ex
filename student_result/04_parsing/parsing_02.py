...
class post:
    ...
    def __init__(self, tag, post_type):
        ...
        self.url = tag[0].replace('" target="_self">', "").replace('<a class="boardList_item" href="', "").strip()
        self.name = tag[1].replace('<span class="hidden-xs">', "").replace('</span>', "").strip()
...
with requests.Session() as s:
    # 세션을 통해 실행한다...
    login_page_html = get_html_with_session(s, "https://go.sasa.hs.kr")  # 로그인 페이지의 html을 가져온다.
    parsed_login_page = parse_html(login_page_html)  # 파싱(문법 해부)

    csrf = parsed_login_page.find('input', {"name": "csrf_test_name"})  # csrf 코드를 찾는다.
    LOGIN_INFO.update({"csrf_test_name": csrf['value']})  # 로그인 정보를 업데이트한다.

    login_req = s.post('https://go.sasa.hs.kr/auth/login/', data=LOGIN_INFO)  # 로그인 정보를 가지고 접속을 시도한다.
    if login_req.status_code != 200:  # 만일 성공적으로 로그인하지 못했다면(200: 성공적으로 불러옴을 의미)
        raise Exception("로그인에 실패하였습니다.")  # 에러를 발생시키고 프로그램 종료.

    main_page_html = get_html_with_session(s, "https://go.sasa.hs.kr/main")  # 메인 페이지의 html 코드를 가져와
    ...
    for i in board_menu:  # 각 줄에 대해 검색
        ...
        if string_tag.split("/")[1] == "board":  # 이 조건문은 해당 탭이 링크를 포함하고 있는지를 확인한다.
            board_id_list.append(string_tag.split("/")[3])  # 보드 id만 얻어와 리스트에 추가

    for i in board_id_list:  # 게시판 종류별로 탐색
        board_url_without_page_num = ("https://go.sasa.hs.kr/board/lists/" + str(i) + "/page/")  # 페이지 번호 제외 주소
        page_num = 0  # 0에서 시작하여, 1, 2, ...

        while (True):  # 페이지 번호의 한계를 모르므로 무한 루프 + break문 이용
            page_num += 1  # 페이지 번호 1 증가
            board_url = board_url_without_page_num + str(page_num)  # 해당 페이지로 이동
...
for i in result_list:  # 하나씩 출력함
    cnt += 1
    print("[" + str(cnt) + "] " + str(i), end=" ")  # __str__이 post 클래스에 구현되어 있음
    # 종류에 따른 구분
    if (i.type == 0):
        print("(중요)")
    elif (i.type == 1):
