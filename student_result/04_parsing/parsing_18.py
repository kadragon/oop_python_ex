...
def sub_get_insert_time_and_press(url): #html에서 판매랭킹 1위부터 10위까지 상품 브랜드, 상품 이름, 상품 가격 가져오는 함수
    ...
    for i in range(0, 10):
        item_brand = sub_soup.select('div.article_info p.item_title a')[i].getText() #상품 브랜드 가져오기
        item_name = sub_soup.select('div.article_info > p.list_info')[i].get('title') #상품 이름 가져오기
        item_price = sub_soup.select('div.article_info > p.price')[i].getText().split()[1] #상품 가격 가져오기
        if item_price == '▼' or item_price == '▲': #예외처리
            item_price = sub_soup.select('div.article_info > p.price')[i].getText().split()[0]
...
def recom_size(n): #관심있는 상품 url 출력 함수
    recom_html = get_html('https://store.musinsa.com/app/contents/bestranking') #무신사 판매랭킹 Top 10 주소 html
    recom_soup = bs4.BeautifulSoup(recom_html, 'html.parser')
    print("관심있는 상품의 url입니다.")
    print("https://store.musinsa.com"+recom_soup.select('div.list_img a')[n-1].get('href')) #관심있는 랭킹의 상품 url 출력



brand_item, name_item, price_item = sub_get_insert_time_and_press('https://store.musinsa.com/app/contents/bestranking') #무신사 판매랭킹 Top 10 받아오기

print("무신사 판매랭킹 Top 10")
for i in range(0, 10): #판매랭킹 Top 10 브랜드, 이름, 가격 출력
    print("Rank %d : %s | %s | %s" %(i+1, brand_item[i], name_item[i], price_item[i]))

while 1 :
    a=input('\n관심있는 상품이 있다면 번호를 눌러주세요 (관심있는 상품이 없다면 0을 입력하세요) ') #관심있는 상품의 번호 물어보기
    if a=='0': #관심있는 상품 없다면
        break #프로그램 종료
