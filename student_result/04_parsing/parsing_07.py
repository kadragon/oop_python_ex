...
def check_valid_player(url):
    ...
    try:
        search_result=result_soup.select('div#players div.search-item-url')[0].getText().split()
    except IndexError:
        return False
...
def get_player_url(url):
    ...
    player_part_url = result_soup.select('div.search-item-url')[0].getText().split()[0]
...
def get_player_data(url):
    ...
    player_height_weight = player_soup.select('div#meta p')[2].getText().split()
...
while True:
    ...
    search_link = "https://www.baseball-reference.com/search/search.fcgi?hint=&search=" + search + "&pid=&idx="
