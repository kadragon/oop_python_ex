...
def hit_num(d,ans,count):          # 데이터와 정답을 비교하는 함수
    ...
    ball_stack  =0  # 볼 수
    for i in range(0,3):   # 각 자릿수를 i번째 정답숫자와 비교한다.
        if a==ans[i]:      # 일치하는 경우 중에서 a는 첫번째, b는 두번째, c는 세번째 자리인지 확인하여 스택 추가
            if i==0:
                strike_stack=strike_stack+1
            else:
                ball_stack=ball_stack+1
        elif b==ans[i]:
            if i==1:
                strike_stack=strike_stack+1
            else:
...
while play_again == 'yes' or play_again == 'y':
    ...
    while count<10:
        ...
        if point==0 or letter>3:            # 하나라도 잘못됨 혹은 문자길이 3개 이상
            ...
            continue
        count=count+1     # 입력이 잘 되었으면 count 추가
