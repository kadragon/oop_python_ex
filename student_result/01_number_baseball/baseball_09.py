...
def find_error(temp):
    if len(temp)!=numberlen : return 0
...
while find_checking(answer)==False :
...
for playtime in range(canplayidx) :
    ...
    if checking <= 2 :
        if checking == 0 : print('ERROR l 숫자는 띄어쓰기가 들어간 세 자리입니다. ㅣ ERROR')
        elif checking == 2 : print('ERROR l 같은 숫자를 입력하지 마시오. ㅣ ERROR')
        print('')
        continue
