...
def this_function_will_draw_the_board(a_long_thin_fl):
    ...
    print('        ==========================    [Numbering]')
    print('                |        |            7 8 9')
    print('            ' + a_long_thin_fl[7] + '  ' + ' | ' + '   ' + a_long_thin_fl[8] + '  ' + ' | ' + '   ' +
          a_long_thin_fl[9] + '       4 5 6')
    print('                |        |            1 2 3')
    print('        --------------------------')
    print('                |        |        ')
    print('            ' + a_long_thin_fl[4] + '  ' + ' | ' + '   ' + a_long_thin_fl[5] + '  ' + ' | ' + '   ' +
          a_long_thin_fl[6] + '  ')
    print('                |        |        ')
    print('        --------------------------')
    print('                |        |        ')
    print('            ' + a_long_thin_fl[1] + '  ' + ' | ' + '   ' + a_long_thin_fl[2] + '  ' + ' | ' + '   ' +
          a_long_thin_fl[3] + '  ')
    print('                |        |        ')
    print('        ==========================')
    print('')
...
def this_function_will_ask_player_to_where_do_you_wanna_check_return_int_player_checked(a_long_thin_fl):
    ...
    wrong_cnt = 0
    while move not in '1 2 3 4 5 6 7 8 9'.split() or not this_function_will_return_the_board_is_empty_return_bool(
            a_long_thin_fl, int(move)):
        wrong_cnt += 1
        # 이상하게 입력한 횟수 5회 초과시
        if wrong_cnt > 5:
...
def this_function_will_determin_where_computer_will_place_return_int_range_1_to_9(a_long_thin_fl, compLetter):
    ...
    # 이길거 같으면 끝내라
    if (this_function_will_return_the_board_is_empty_return_bool(copy, 1)) and (
            compLetter == copy[2] == copy[3] or compLetter == copy[5] == copy[9] or compLetter == copy[4] == copy[7]):
        return 1
    elif (this_function_will_return_the_board_is_empty_return_bool(copy, 2)) and (
            compLetter == copy[1] == copy[3] or compLetter == copy[5] == copy[8]):
        return 2
    elif (this_function_will_return_the_board_is_empty_return_bool(copy, 3)) and (
            compLetter == copy[1] == copy[2] or compLetter == copy[6] == copy[9] or compLetter == copy[5] == copy[7]):
        return 3
    elif (this_function_will_return_the_board_is_empty_return_bool(copy, 4)) and (
            compLetter == copy[1] == copy[7] or compLetter == copy[5] == copy[6]):
        return 4
    elif (this_function_will_return_the_board_is_empty_return_bool(copy, 5)) and (
            compLetter == copy[1] == copy[9] or compLetter == copy[2] == copy[8] or compLetter == copy[3] == copy[
        7] or compLetter == copy[4] == copy[6]):
        return 5
    elif (this_function_will_return_the_board_is_empty_return_bool(copy, 6)) and (
            compLetter == copy[4] == copy[5] or compLetter == copy[3] == copy[9]):
        return 6
    elif (this_function_will_return_the_board_is_empty_return_bool(copy, 7)) and (
            compLetter == copy[1] == copy[4] or compLetter == copy[5] == copy[3] or compLetter == copy[8] == copy[9]):
        return 7
    elif (this_function_will_return_the_board_is_empty_return_bool(copy, 8)) and (
            compLetter == copy[7] == copy[9] or compLetter == copy[5] == copy[2]):
        return 8
    elif (this_function_will_return_the_board_is_empty_return_bool(copy, 9)) and (
            compLetter == copy[1] == copy[5] or compLetter == copy[3] == copy[6] or compLetter == copy[7] == copy[8]):
        return 9
    # 먼저 귀
...
while True:
    ...
    while is_the_game_was_end_or_not_end_a_Ha_its_commencing_now:
        ...
        if turn == 'Player':
            ...
            move = this_function_will_ask_player_to_where_do_you_wanna_check_return_int_player_checked(
                this_value_mean_the_plate_which_is_game_)
            this_function_make_move_and_its_not_return_anything(this_value_mean_the_plate_which_is_game_, playerLetter,
                                                                move)

            # 플레이어가 이겼다면
            if this_function_determin_the_letter_won_or_not_return_bool(this_value_mean_the_plate_which_is_game_,
                                                                        playerLetter):
                this_function_will_draw_the_board(this_value_mean_the_plate_which_is_game_)
        ...
        # 컴퓨터 턴
            ...
            move = this_function_will_determin_where_computer_will_place_return_int_range_1_to_9(
                this_value_mean_the_plate_which_is_game_, compLetter)
            this_function_make_move_and_its_not_return_anything(this_value_mean_the_plate_which_is_game_, compLetter,
                                                                move)
    ...
    # 계속할 지 물어봅니다
    if this_function_ask_user_to_want_to_play_again_return_bool_true_or_false(
            this_function_determin_the_letter_won_or_not_return_bool(this_value_mean_the_plate_which_is_game_,
                                                                     compLetter)):
        break
