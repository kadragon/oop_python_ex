...
def start_notification():
    ...
    if rule in "y yes".split():        # y, 또는 yes가 입력될 경우 규칙에 대한 설명 제공
        rule_explain()
    ...
    print('\n' + '=' * 100)
    print("Enter 를 누르면 시작합니다!", end=' ')
    input()  # Enter 를 누른 후에 게임이 시작되도록 임의의 입력을 받음.
...
def random_number():
    ...
    num_list = list(range(10))                      # 0부터 9까지의 정수 리스트 선언
...
def is_right_input(number):
    ...
    :return: 옳은 형식의 입력일 경우 True, 옳지 않은 형식의 입력일 경우 False 를 반환.
    """
    ...
    for i in number:        # number 의 모든 구성 문자에 대해 확인.
        if i not in digits:     # number 의 구성 문자가 정수 문자가 아닐 때,
            print("영문, 한글, 특수문자, 공백 등의 숫자가 아닌 문자는 입력할 수 없습니다.\n")   # 입력 오류 안내
            return False    # 옳지 않은 입력이므로 False 반환.

    if len(number) < NUM_DIGITS:    # number 의 자릿수가 정답 숫자의 자릿수보다 작을 때
        print("입력하신 숫자의 개수가 너무 적습니다. %d개의 숫자를 입력해 주십시오.\n" % NUM_DIGITS)    # 입력 오류 안내
        return False    # 옳지 않은 입력이므로 False 반환.

    if len(number) > NUM_DIGITS:    # number 의 자릿수가 정답 숫자의 자릿수보다 클 때
        print("입력하신 숫자의 개수가 너무 많습니다. %d개의 숫자를 입력해 주십시오.\n" % NUM_DIGITS)    # 입력 오류 안내
        return False    # 옳지 않은 입력이므로 False 반환.

    for i in number:    # number 의 모든 구성 문자에 대해 확인.
        if check[int(i)] == 1:      # 같은 숫자가 2번 number 에 두 개 이상 존재할 때,
...
def baseball_information(number):
    ...
    :return: 플레이어가 입력한 정수 문자열이 정답과 일치할 경우 True, 일치하지 않을 경우 False 를 반환.
    """
    ...
    for i in range(NUM_DIGITS):     # 0 ~ NUM_DIGITS-1 의 인덱스, 즉, 문자열의 모든 문자에 대해 확인.
        if number[i] == ANSWER[i]:  # number 에 속한 문자와 정답 문자의 종류와 위치가 모두 같은 경우,
            strike += 1             # strike 개수 증가
        elif number[i] in ANSWER:   # number 에 속한 문자와 정답 문자의 종류는 같지만 위치가 다른 경우,
            ball += 1               # ball 개수 증가
        else:                       # number 에 속한 문자가 정답 문자열에 존재하지 않는 경우,
            out += 1                # out 개수 증가

    print("\nStrike → %d | Ball → %d | Out → %d" % (strike, ball, out))       # '숫자 야구' 형식의 정보 안내.

    if strike == NUM_DIGITS:    # 스트라이크의 개수와 정답 숫자의 자릿수가 같은 경우, 즉, number 가 정답 문자열인 경우,
        return True             # number 가 정답이므로 True 반환.
    else:                       # number 가 정답 문자열이 아닌 경우,
        return False            # number 가 오답이므로 False 반환.
...
def play_again():
    ...
    if replay in "y yes".split():    # y 또는 yes가 입력될 경우, 플레이어가 재시작을 원한다는 의미의 True 리턴.
        return True
