...
def scan_number (count) :
    ...
    return user
'''
count번째 사용자의 입력을 받아 list형으로 반환하는 함수
'''
...
'''
사용자의 값 num과 정답 ans의 스트라이크, 볼, 아웃 개수를 세는 함수이다.
만약 num과 ans가 일치한다면 정답이라고 쓰인 문자열을 반환한다.
...
while True:
    ...
    while count <= 10 :
        ...
        check_number(guess) # guess가 양식에 맞는지 확인함
        print(check(answer, guess)) # guess와 answer의 관계 출력
        count += 1# 시도 횟수 증가

        if answer == guess :
            break# guess와 answer이 동일할 경우 멈춤
