...
def scan(i):  # 사용자가 입력한 문자열을 받아 입력 조건에 부합하는지 판단하는 함수
    ...
    if right(t):  # 입력 조건에 부합하는 문자열이라면 gamestart 함수로 리턴
        return t
    else:
...
def gamestart():  # 게임을 진행하는 함수
    num = []  # (line 31 ~ 42) 컴퓨터가 생각한 세자리 수 abc 를 num 에 [a, b, c]형태로 저장 (a, b, c : int)
    print('게임을 시작합니다.')
    ...
    for i in range(1, 11):  # 시도 횟수를 10번으로 제한
        num2 = []  # (line 46 ~ 49) 사용자가 입력한 수 abc 를 num2에 [a, b, c]형태로 저장 (a, b, c : int)
        k = scan(i)
        for i2 in range(3):
