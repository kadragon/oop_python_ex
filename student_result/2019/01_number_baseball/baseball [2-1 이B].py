...
def check(numlist):   # 리스트를 받아 입력이 조건에 맞는지 검사한다. 조건에 맞지않는다면 False 를 리턴한다.
    ...
    randnumlists = "0123456789"  # 들어갈 수 있는 수들이 모두 들어있는 리스트
    ...
    for i in range(3):
        ...
        #     return False
        if numlist[i] not in randnumlists:
            return False
...
def createrandlist():  # 무작위 세자리 수를 생성하는 함수
    numberlist = list("0123456789")     # 중복없는 0부터 9까지의 문자열을 만든다.
...
if __name__ == '__main__':
    ...
    while yesorno is 'Y':
        ...
        while len(trylist) < 10:  # 올바른 숫자열 한번당 trylist에 원소가 하나씩 추가되므로 10번의 기회를 주는것과 같다
            ...
            tri = list(input())  # 숫자열 입력부분. 문자열을 받아 리스트로 받게된다.
        ...
        if FS == 'Success':  # 성공 여부에 따라 나오는 멘트가 다르다
            ...
            print("정답은 '" + str(randnumlist)[0:3] + "' 이었습니다.")
