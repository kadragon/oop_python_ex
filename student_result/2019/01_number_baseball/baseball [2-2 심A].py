...
def makenum():  # 0~9의 수 중 랜덤한 수 세개를 만들어, 그 수들을 리스트로 만들어 리턴한다.
    # sourcelist = []  # 0~9까지 숫자를 넣은 다음 shuffle 할 리스트
    # for i in range(10):
    #     sourcelist.append(i)
    sourcelist = list(range(10))
...
def trygame(makelist: list, uselist: list, flag1: int):  # 사용자가 입력한 숫자 3개를 입력받아, 결과를 출력하고, 다음으로 실행될 횟수를 리턴
    # makelist: 컴퓨터가 생각한 숫자들의 리스트  # T. makeList 와 같이 명명법을 사용하는것이 좋음.
    ...
    for i in range(3):
        tmpflag = 0  # strike 나 ball 를 증가시킨다면 1, 아니면 0
        for j in range(3):
            if uselist[i] == makelist[j]:  # 사용자가 입력한 숫자와, 컴퓨터가 정한 숫자가 맞는지 비교
                ...
                if i == j:  # 위치가 맞는지 비교
                    ...
                    ball += 1
        if tmpflag == 0:  # strike 나 ball 이 증가되지 않았음: 숫자와 위치가 틀림
            out += 1
