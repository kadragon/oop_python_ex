...
def answer():
    ...
    random.shuffle(n)  # list 인 n 내부의 값들을 임의의 순서로 섞어준다.
    num = ''
    for i in range(3):  # 숫자길이인 3 만큼 반복하며 길이3의 문자열 num 을 만듬 / 이렇게 만들경우 중복이 생기지 않는다.
        num += str(n[i])
    return num
...
def choosinglife():
    ...
    """
    # while 1:  # return 이 안됬다면(MAX_LIFE 가 숫자로만 안이루어졌을 때) 계속 반복
    while True:  # return 이 안됬다면(MAX_LIFE 가 숫자로만 안이루어졌을 때) 계속 반복
        print("=" * 80)
        # T. python 에서 대문자로만 이루어진 변수명은 '상수' 를 의미한다.
        MAX_LIFE = input('먼저 목숨을 몇개 가질지 정해주세요 : ')  # 사용자가 목숨의 개수를 입력한다. (문자열로)
        if MAX_LIFE.isdigit():  # 문자열이 숫자로 이루어졌을때만 if 조건을 통과시켜주는 함수 .isdigit()을 사용하였다.
            print("숫자를 %s번 안에 맞춰주세요" % MAX_LIFE)
            print("=" * 80)
            MAX_LIFE_n = 0
            for k in MAX_LIFE:  # 반복문을 사용하여 정수형인 MAX_LIFE_n에 한자리씩 MAX_LIFE 의 값들을 대응시킨다. / 문자열'123'-->숫자 123, '092'-->92
                MAX_LIFE_n *= 10
                MAX_LIFE_n += int(k)
...
def isitright(n):
    ...
    """
    if n.isdigit():  # n 이라는 문자열이 숫자로만 이루어져있을때만 if 조건을 통과시켜주는 함수 .isdigit()을 사용하였다.
        if len(n) == 3:  # n 이라는 문자열의 길이가 3일때만 if 조건 통과
            return True
        else:
            return False
    else:
...
def makingsbo(guess, num):
    ...
    if guess == num:  # 사용자가 숫자를 정확히 맞추었을 때
        ...
        for i in range(3):  # guess 가 3자리 문자열이니 3번반복하며 guess 의 숫자들을 모두 분석한다.
            if guess[i] == num[i]:  # guess 와 num 의 i번째 숫자가 자리와 값이 모두 같을 경우 strike+1
                count_strike += 1
            elif guess[i] in num:  # guess 의 i번째 숫자가 num 문자열 안에 들어있지만 자리가 같진 않을때 ball+1
                count_ball += 1
            else:  # 위에 두가지 경우가 모두 아닐 때(guess 의 i번째 숫자가 num 에 없다) out+1
                count_out += 1
...
def ending():
    """
    사용자가 숫자를 맞추거나 목숨을 다써서 게임 한판이 끝났을 때
    사용자에게 게임을 재시작할건지 종료할건지 선택하게 하는 함수
    :return: 사용자가 게임을 종료한다면 False, 게임을 다시 시작하겠다고 하면 True
    """
    while 1:  # return 이 안됬다면(질문에 yes,no로 대답안했을 때) 계속 반복
        print("=" * 80)
        a = input('게임을 다시 하시겠습니까? (yes or no)')  # a는 첫번째 질문에 대해 사용자가 입력한 것
        if a == 'no':  # 사용자가 게임을 다시 안하겠다고 했을 때
            # while 1:  # return 이 안됬다면(질문에 yes,no로 대답안했을 때) 계속 반복
            while True:  # return 이 안됬다면(질문에 yes,no로 대답안했을 때) 계속 반복
...
while 1:
    ...
    num = answer()  # num 은 사용자가 맞추어야하는 3자리 숫자
    while attempt <= MAX_LIFE:  # 목숨보다 시도 횟수가 작거나 같을 때 계속 반복
        ...
        if isitright(guess):  # 사용자가 입력한 것이 게임 조건에 맞는 3자리 정수인지 판단해서 3자리 정수가 맞다면,
            s = makingsbo(guess, num)  # guess 와 num 이 같다면 s는 True, 아니면 s에 값이 반환이 안된다.
            if s:  # guess 와 num 이 같다면 게임 종료
                break
            attempt += 1  # 시도횟수 1증가
            if MAX_LIFE - attempt + 1 == 0:  # 현재남은 목숨이 0이 되면 메세지와 함께 while 조건 벗어나며 게임종료
                print("\n실패하였습니다ㅜㅜ\n")

        else:  # 사용자가 3자리 정수말고 다른 것을 입력하였을 때
            print("제대로 입력해주세요ㅜㅜ")
    a = ending()  # 한 게임이 끝났으니 사용자에게 게임을 다시할 것인지 종료할 것인지 선택하게 하는 함수 실행
    if a:  # 사용자가 게임을 다시 한다고 하였을 때
        continue
