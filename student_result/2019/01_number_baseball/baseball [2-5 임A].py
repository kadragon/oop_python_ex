...
def input_check(list): # 사용자의 입력값을 리스트로 받아서 조건에 충족하는지 확인하는 함수이다. 에러가 없으면 1을, 있으면 0을 리턴.
    ...
    for i in list: # 매개변수로 받은 리스트 내의 문자열을 모두 체크한다.
        ...
        for j in list[cnt:]: # 리스트의 현재 위치 이후의 값들 중
            if i == j: # 현재 값과 같은 것이 있다면
                err_set.add('rept') # err_set에 '중복 오류'가 발생했다고 저장한다.

    if cnt is 0: # 만약 사용자가 아무 입력도 하지 않았다면
        err_set.add('null') # err_set에 '공백 오류'가 발생했다고 저장한다.
    elif cnt is not 3: # 만약 사용자 입력값의 길이가 3이 아니라면
        err_set.add('size') # err_set에 '길이 오류'가 발생했다고 저장한다.
    ...
    if cnt is 5: # 숫자 사이에 공백을 넣은 사용자들을 위한 에러이다. 공백을 포함해 총 5개의 원소가 있을 것이다.
    ...
    end_cnt = len(err_set) # end_cnt는 현재 사용자의 입력값 중에서 에러가 몇 종류나 있는지를 저장한다.
    if len(err_set): # 만약 에러가 하나라도 있다면
        print('입력 오류:', end = ' ') # 입력에서 오류가 있다고 출력한 뒤

    for i in err_set: # 발생한 에러의 종류들이 하나씩만 저장된 err_set의 원소들마다
        end_cnt -= 1  # err_cnt를 아직 탐색하지 않은 err_set의 원소의 수로 정하고
        if end_cnt == 0:  # 만약 이를 제외하고 에러가 더 있지 않다면
            end_chr = ' 입력하세요.\n' + '=' * 80 + '\n'  # 에러 출력을 마무리한다.
        print('%s' %(err_dict[i]), end = end_chr)  # 딕셔너리 err_dict에서 각각 에러명에 해당하는 에러 알림을 출력한다.

    err_cnt = len(err_set) # 발생한 에러 종류의 수를 err_set의 원소 개수로 정한다.
...
def correct_chk(attemp, ans): # 시도 횟수와 사용자 입력값을 전해받아 S, B, O를 출력하고 정답 여부를 1, 0으로 리턴한다.
    ...
    while not chk: # 사용자 값이 입력조건을 만족하지 않는 동안
        ...
        chk = input_check(guess) #input_check 함수를 통해 입력조건을 만족하는지 확인한다. 여기서 조건을 만족하면 chk값이 1이 되어 while문을 빠져나간다.
...
while replay:
    ...
    while not correct: # 정답이 맞지 않는 동안
        if (correct_chk(attemp, ans)): # 만약 이번 추측 값이 정답이라면
            ...
            score_list.append([attemp, input()]) # 순위 등록을 위해서 이름을 입력받고, 시도 횟수와 함께 score_list에 저장한다.
            break # 이번 게임을 끝낸다.
    ...
    while 1:
        replay_input = input()
        if replay_input in yes_list: # 만약 다시 플레이할지 물어봤을 때 입력값이 yes_list에 있다면
        ...
        elif replay_input in no_list: # 만약 다시 플레이할지 물어봤을 때 입력값이 no_list에 있다면
            if replay_input != 'no': # 만약 no는 아니라면
                print("'%s'도 좋지만 다음부터는 시키는대로 입력하세요..." %(replay_input)) # 특별 메시지를 출력한다.
            replay = 0 # 다시 플레이할지의 여부를 0으로 설정한다.
            break # 다시 플레이 할지의 여부 입력 과정을 나간다.
        else:
            print('yes 또는 no로 입력해주세요.', end = ': ') # 다시 플레이할지 물어봤을 때 입력값이 yes_list나 no_list에 없다면 다시 입력받는다.

sorted(score_list, key = lambda x: x[1], reverse = 1) # 사용자가 다시 플레이하지 않는다면 score_list의를 각 원소의 attemp 횟수의 오름차순으로 정렬한다.
print('=' * 80)
print('순위표')
print('순위 │ 시도 횟수 │ 이름')
