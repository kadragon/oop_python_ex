...
def hint(guess, ans):
    """
    입력값에 대한 힌트를 제공하는 함수
    매개변수 : guess 는 사용자 입력값, ans 는 정답을 뜻함
...
def isNotRight(data):
    ...
    elif data.isdecimal() is False:  # 숫자가 아닌 문자가 포함된 경우
        return True
...
while True:
    ...
    while replayInput(res):  # 입력이 제대로 이루어지지 않은 경우
        ...
        if replayInput(res) is False:  # 다시 올바른 값이 입력되면 while문 탈출
            break
