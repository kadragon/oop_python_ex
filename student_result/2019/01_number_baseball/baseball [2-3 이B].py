# 컴퓨터가 랜덤한 세자리수를 생성하기 위해 random 을 import 해줍니다
import random
import time
...
def make_number():
    ...
    answer_num = ''  # 사용자가 맞추어야할 정답인 answer_num 을 초기화시켜줍니다
    for i in range(3):  # 무작위로 섞은 num_list 의 앞의 세  수를 활용해 세자리수 answer_num 을 만듭니다
...
def strike_ball_out(try_num, answer_num):
    """
    사용자가 예측한 세자리수인 try_num 과 정답인 answer_num 을 비교하여 규칙에 맞게 strike, ball, out 의 개수를 알려줍니다
    """
...
def right_number(insert):
    ...
    """
    er = 0  # T. 변수명 선정시에 너무 줄인 축약어를 사용지 말길

    if insert == ' ':
        return False

    for i in insert:
        if not i.isdigit():  # 사용자가 입력한 값 중 하나라도 숫자가 아니라면 False 를 return 합니다
            er += 1
    if er != 0:
        print("/ 0 에서 9까지 숫자로 구성되어야 한다니까", end=' ')

    if len(insert) != 3:  # 사용자가 입력한 수가 세자리수가 아니라면 False 를 return 합니다
        print("/ 세자리 정수를 입력하라고;;", end=' ')
        er += 1

    elif insert[0] == insert[1]:  # 사용자가 입력한 값 중 중복된 값이 있다면 False 를 return 합니다
...
print("=" * 70)
print("숫자야구에 온 것을 환영한다! 나는 오직 숫자야구를 위해 만들어진 alpha-num_baseball 이지\n")
