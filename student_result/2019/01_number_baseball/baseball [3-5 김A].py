...
def creating_answer():
    ...
    random.shuffle(numbers)  #numbers의 수 순서를 바꿈"""
    answer = '' #정답(answer) 변수 선언
    for i in range(3):
        answer += str(numbers[i]) #answer가 바뀐 numbers의 앞 세 수
    while answer[0] == '0': #answer 첫 수가 0이면 다시 answer 만듦
        answer = '' #answer를 초기값 공백으로 지정
        random.shuffle(numbers) #numbers의 수를 다시 섞음
        for i in range(3):
            answer += str(numbers[i]) #answer 다시 지정
    return answer #answer값 반환
def count():
    print('%s번 잘못 입력했고, 남은 기회는 %s번입니다' %(c,life)) #c번 잘못 입력했고, 남은 기회는 life번입니다 (c와 life는 변수)
def int_test(num):
    if num == 'rules': #num이 rules이면 rules 함수를 실행하고 True 반환
        rules()
        return True
    elif num == 'count': #num이 count이면 rules 함수를 실행하고 True 반환
        count()
        return True
    elif len(num) != 3: #num의 자릿수가 3이 아니면 False 반환
        return False
    else:
...
def ans_test(user_guess, com_ans):
    ...
    o = 0 #out 수 =0
    for i in range(3):
        if user_guess[i] == com_ans[i]: s += 1 #user_guess의 i+1번째 수=com_ans의 i+1번째 수면 strike 하나 늘림
        elif com_ans.find(user_guess[i]) != -1: b += 1 #user_guess의 i+1번째 수가 com_ans에 있으면 ball 하나 늘림(strike인 경우 제외)
        else: o += 1 #그 외에는 out 하나 늘림
    print('%sS | %sB | %sO' %(s,b,o)) #sS | bB | oO 출력(s,b,o)변수
def again():
    print('게임을 다시 하시겠습니까? 다시 하려면 yes를 입력하세요') #~출력
    a = input() #입력받음
    if a == 'yes': return True #입력값이 yes면 True 반환
    else: return False #나머지면 False 반환
...
print("\n규칙을 확인하려면 rules를 입력하세요") #출력
print("틀린 횟수와 남은 기회를 확인하려면 count를 입력하세요") #출력
...
while True:
    ...
    while life > 0: #life 가 0보다 클 때
        ...
        while int_test(guess)==False and life > 0 : #올바르지 않은 수 또는 문자를 입력했을 때, 이 while문을 돌 때 life 확인을 위해 life>0 필요
            print('세 자리 자연수를 입력하세요') #출력
            if c >= 1: # 잘못 입력한 횟수가 한번보다 많으면 life 차감
                life -= 1
            c += 1 # 잘못 입력한 횟수 하나 추가(이 경우는 틀린 경우니까 추가)
            if life == 0: #life가 0이되면 이 while문에서 나감
                break
            else: guess = input() #life가 0이 아니면 새로운 guess값을 입력받음

        try:
            if guess != 'count' and guess != 'rules': #guess가 count 또는 rules가 아니고
                if int_test(guess): #guess가 세 자리 정수 일 때 life 차감(정답이 아닌 것을 입력했을 때 life 차감)
                    life -= 1
                if guess == answer:
        ...
        except Exception: #공백 또는 enter를 입력받았을 때 생길 수 있는 에러 해결
            continue
