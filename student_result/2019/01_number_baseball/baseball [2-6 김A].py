...
세자리 숫자를 입력하면, Strike, Ball, Out의 개수가 출력됩니다. 
...
def raw_input():        # 목적 : 사용자의 입력을 프로그래밍에 맞게 바꿔주는 함수
    ...
    cheat = False
    try:
        base_input = []  # 사용자의 입력을 list로 바꾸어 정리할 공간
        num = 100  # 세 자리 숫자를 한 자리씩 구분하기 위한 변수
        input_type = 0  # input된 값이 띄어쓰기로 구분되었는지 세자리수의 정수인지 확인하기 위한 것
        ...
        if input_type == 0 and len(a) == 3:     # 띄어쓰기로 구분되어있는데 숫자라면 이렇게 처리함
            ...
            for i in range(3):
                ...
                base_input.append(str(int(a / num)))    # 한자리씩 분리하고 list에 넣음
...
def baseball_input():  # 사용자의 입력을 받음. 123의 형태 혹은 1 2 3의 형태를 제외하고는 재입력을 요구함
    ...
    while 1:
        val = raw_input()
        if val is not False:        # False가 전달되지 않았다면 제대로 입력된 것이므로
            return val              # 전달함


def baseball_game_unit(answer, base_input):     # 사용자 한번의 입력에 대해 SBO를 구분해주는 최소단위임. SBO를 전달함
...
def print_SBO(ans):         # Strike, Ball, Out을 표시하는 함수. 맞추었는지도 판정
    ...
    try:
        ...
        Strike, Ball, Out = baseball_game_unit(ans, b_input)
...
def game_ended(answer: list):     # 맞추지 못하고 게임이 끝났을 때 답과 위로메세지를 표시하는 함수 list형의 입력을 받음
    print("Answer is", end=' ')
    for i in answer:
        print(i, end='')
    print()
...
def yes_or_no():        # 게임 재시작에 대한 입력을 판별하는 함수
    try:
        ...
        if r in yes_list:                   # 만약 재시작을 원하면
            yes_or_no_list.append(True)     # True를 list를 넣어주고
        elif r in no_list:                  # 재시작을 원하지 않으면
            yes_or_no_list.append(False)    # False를 list에 넣어준다
        val = yes_or_no_list[0]             # 만약 yes도 no도 아니라면 yes_or_no_list = []일 거고 이 명령을 실행하면 error가 남
        return val
    except Exception as inst:               # error가 났다는 것은 재입력이 필요하단 말임
        print("Error :", inst)              # error의 종류를 표시하고
...
def ask_try_again() -> bool:                    # 다시 할 것인지 물어보는 함수
    ...
    while 1:
        reply = yes_or_no()
        if type(reply) is bool:         # reply의 type이 bool이 아니라면 1이 전달된 것이고 다시 물어보아야 한단 뜻임
            return reply                # 제대로 입력되었다면 원하는 대로 실행하기 위해 찬성반대를 전달함


while retry:                            # retry는 기본적으로 True인 bool형 인자임
    start_action()                      # 시작과 함께 규칙설명. 재식작하면 다시 나옴
    right = False                       # type: bool
    ans = make_number()                 # 랜덤으로 만들어진 답 list형임
    ...
    retry = ask_try_again()         # 찬성반대를 받았고 재시작을 원하지 않으면 retry가 False가 되어 작동을 종료함
