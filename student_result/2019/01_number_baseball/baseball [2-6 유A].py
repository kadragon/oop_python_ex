...
def make_key(digit):  # key 값을 만드는 함수
    ...
    for i in range(digit):  # 설정한 자리수 만큼 숫자를 리스트에서 가져온다
        key += str(num[i])  # key 를 만든다

    return key  # key 를 리턴한다


def result(user_input, key):  # strike, ball, out 을 판정 하는 함수 , user_input 은 사용자의 입력값
    ...
    # return str(cnt_strike) + ' strike, ' + str(cnt_ball) + ' ball, ' + str(
    #     cnt_out) + ' out\n'  # strike, ball, out 결과 리턴
    return "%d strike, %d ball, %d out\n" % (cnt_strike, cnt_ball, cnt_out)


def correct_input(x):  # 사용자가 정확하게 입력했는지 확인하는 함수
    if x == ' ':  # 비어있으면 False 를 리턴한다
        return False

    for i in x:  # 입력에 0~9 까지의 숫자 이외의 문자가 있으면 False 를 리턴한다
        if i < '0' or i > '9':
            return False

    for i in range(0, len(x)):  # 사용자의 입력에 중복되는 숫자가 있으면 False 를 리턴한다
...
def play_again():  # 다시 플레이 할 지를 판단하는 함수
    return input('Do you want to play again? (y / else)').lower() == 'y'  # y가 입력되면 True 를 리턴하고 그 이외에는 False 를 리턴한다
