...
def starter(again):     #게임을 실행 여부 묻는 함수, again은 처음하는 건지 아니면 다시하는 건지에 대해 알려주는 변수
    while 1:
        if again == 1:    #다시하는 거임
...
while 1:
    if cnt == 0: #cnt가 0이면 패배로 간주한다
        ans = 100*num[0] + 10*num[1] + num[2]
        print("이런 맞추기에 실패하셨네용~~ 정답은 ",ans,"였습니다~")
        again = 1 #again과 x를 1로 바꾸어서 다시 하는 경우로 바꾸고, 새로 진행할 수 있도록 한다
        x = 1
        cnt = 10
        print("------------------------------------------")
