...
def build():  # 0부터 9까지의 정수 중 겹치지 않게 세 개의 정수를 고른다.
    ...
    for _ in range(3):  # num 에 임의의 정수 세 개를 넣는다.
...
def game():             # 사용자에게 세 개의 정수를 입력받아 스트라이크인지 볼인지 아웃인지 판단한다.
    global s, b, o
    a = list(input())   # a는 사용자의 입력값  T. 의미있는 변수명을 사용하는 것을 권장합니다.

    while ' ' in a:     # 사용자 입력 공백 제거
        a.remove(' ')

    if len(a) == 3:     # 사용자가 입력한 리스트의 길이가 3이라면
        # 세 원소 중 하나라도 정수가 아니면 함수 재실행
        if a[0].isdecimal() is False or a[1].isdecimal() is False or a[2].isdecimal() is False:
            print("정수를 입력해주세요!")
            game()
    ...
    for i in range(3):
        if a[i] == num[i]:  # num 과 숫자와 위치가 같을 때 s 증가
            s = s + 1
    for i in range(3):  # num 과 숫자는 같지만 위치가 다를 때 b 증가
    ...
    o = 3 - (b + s)  # num 과 숫자와 위치가 모두 다르면 o 증가
...
def restart():  # 게임이 모두 끝난 후 다시 시작하는 동작을 결정하는 함수
    ...
    if len(answer) == 1:
        ...
        if answer in "yY":  # y나 Y가 입력되었을 때 재실행
            intro()
            build()
            game()
        # elif ord(answer[0]) == 110 or ord(answer[0]) == 78:  # n이나 N이 입력되었을 때 종료
        elif answer in "nN":  # n이나 N이 입력되었을 때 종료
            print("안녕히 가세요!")
            sys.exit()
        else:
            print("y나 n을 입력해주세요!")
    ...
    if len(answer) == 3:
        ...
        if answer in "yes,Yes,yEs,yeS,YEs,YeS,YEs,YES".split(','):
            intro()
            build()
            game()
        else:
    ...
    if len(answer) == 2:
        ...
        if answer in "no,No,nO,NO".split(','):
            print("안녕히 가세요!")
            sys.exit()
        else:
            print("y나 n을 입력해주세요!")
