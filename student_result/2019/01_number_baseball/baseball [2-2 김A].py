...
import random  # Random 수를 생성하기 위해, import random!

Text = """
========================**'s 숫자야구 ========================
-  설명
저는 임의의 세 자리 숫자를 떠올릴 거고요, 플레이어가 그 숫자를 맞추시면 돼요!
세 자리 숫자를 입력하시면, 제가 Strike, Ball, Out 의 개수를 알려드릴게요!
...
플레이어께서 123을 입력하셨다면, 
1은 Strike, 3은 Ball, 5는 Out 이므로
S:1 | B:1 | O:1
라고 알려드릴게요!
"""


def notnum(letter):  # notnum 은 입력문자열이 숫자로만 이루어져있는지 확인한다
    ...
    return 0
    # T. 숫자로만 이루어져 있을때에도 return 을 해주는 것이 좋다.


def input_valid(G):  # T. 변수의 명을 상수로 쓰고 싶을때에 대문자로 쓴다.
    """
    사용자가 입력한 Guess가 적합한지 확인하는 함수이다.
    G를 변수로 받고, 자리수, 숫자여부를 확인한다.

    T. Python 에서 0은 거짓을 1은 참을 나타내긴 하지만
    T. 명시적으로 True, False 을 사용하는 것을 권장한다.
    """
...
def check(G):
    # Strike = Ball = Out = 0  # Strike, Ball, Out 을 초기화한다
    Strike = 0  # T. Python 에서 변수의 첫글자는 소문자를 권장한다.
    Ball = 0    # T. 대문자는 상수로 쓰고 싶을때 사용한다.
    Out = 0
    """
    사용자의 추정에 대한 Strike, Ball, Out 을 판단하여 출력하는 함수. 
    정답(3 Strike)이면 1을 반환한다. 
    G는 사용자의 Guess : G는 리스트로, G[0], G[1], G[2]에 각각 1, 2, 3의 자리 숫자가 담겨있다.
    Checklist 에는 Random 숫자가 저장되있는데, 숫자가 456이라면 
    Checklist 는 [0, 0, 0, 1, 2, 3, 0, 0, 0, 0]으로 저장된다. (각 index 가 사용된 자리를 의미)
    i는 0부터 2까지 순서대로, G[i] (입력 값)이 Random 수에 있는지 확인한다.
    G[0]은 첫 번째 수로, checklist[int(G[0]]이 1이면 Random 에서도 첫 수이므로 Strike
...
while Play:
    ...
    Random 수를 생성하는 부분
    Num 에 랜덤수를 생성하고
    Checklist 는 Num 의 다른 표현법, 사용된 위치를 저장한다 (이후 S,B,O를 판단할때 사용)
    """
    ...
    for i in range(1, 4):
        Num.append(Q_list[i])
        checklist[Q_list[i]] = i  # RANDOM 수에 대한 다른 표현법. checklist 에 사용된 위치를 저장

    print('=' * 60)

    flag_answer = 1  # flag_answer 는 사용자가 정답을 맞추면 0으로 바뀌는 flag
    try_num = 1  # 시도 횟수를 담음
    ...
    while again not in 'ny':  # 재도전 입력이 이상한지 검사
        print('잘못 입력하셨습니다.')
