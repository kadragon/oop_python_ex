...
q_list = ['은기는 정보충입니까?', '강동욱 선생님은 잘생겼습니까?', '이 게임은 재미있습니까?', '태민이는 학점이 4.3입니까?', '동혁이는 빨간색 옷이 어울립니까?',
          '당신은 잘생겼습니까?', '덕용이는 물리를 잘합니까?', '예담이는 께북이를 키웁니까?', '윤정이는 과자를 좋아합니까?', '박정민은 정보에 미쳤습니까?',
          '준아는 머리가 동그랍니까?',
          '동혁이의 키는 155 이하입니까?', '문광식 선생님은 잘생겼습니까?', '박정연 선생님은 멋지십니까?', '도서관은 공부하기 편한 곳입니까?', '급식이 맛있습니까?',
          '양우현은 음치입니까?',
          '양윤정은 춤을 잘 춥니까?', '강준아는 화학을 잘합니까?', '동혁이는 보건실 마스터입니까?']
a_list = ['yes', 'yes', 'yes', 'no', 'no',
          'no', 'no', 'yes', 'yes', 'yes',
          'yes', 'yes', 'yes', 'yes', 'no',
          'no', 'no', 'yes', 'yes', 'yes']
name_reply = [["홍은기", "오, 그 정보충! 어쩐지 이상한 걸 많이 시도하더라"], ["양윤정", "재밌었어, 탱탱볼?"], ["강동욱", "선생님 사랑해요 감점만은 하지 말아주세요"],
              ["김동혁", "어? 왠지 제 컴퓨터 카메라로는 아무것도 안보이더니 동혁님이었군요!"], ["신예담", "청경채 좋아하는 밥돌이..."], ["강준아", "땅콩은 화학을 잘해"]]
memo_list = []
...
def quiz():  # 깜짝 퀴즈를 내는 함수
    if random.randint(1, 6) == 1 and mouth == True:  # 1/6 확률로 깜짝 퀴즈를 진행
...
def select_gamenum():  # 생각할 숫자 개수를 정함
    ...
    # 몇 자리수로 할 것인지를 입력받음
    str1_flag = 1
    while str1_flag == True:  # flag값이 1이면 계속 입력받게 하였음
        str1_flag = False
        str1 = input()
        eq_print()
...
def input_life():  # 목숨을 입력받음, 특별한 목숨 숫자는 선택하여 이스터에그로 만듦
    ...
    while life_temp == None:  # 목숨에 정상적인 입력이 들어올 때까지
        try:
            print("목숨 : ", end='')
            life_temp = int(input())
        except ValueError:  # 숫자가 아닌 입력을 했을 때 발생하는 오류를 except함
            eq_print()
            print("숫자만 똑바로 입력하세요 ^^")
        eq_print()
...
def player_ans2():  # 정상적인 '시도'를 받음, 여기서 숫자를 입력하느냐, hint나 watch 등의 명령어를 입력하느냐가 따로 나뉨
    global hint_num, re_flag, mouth, hint_flag
    # 수 입력 과정
    flag = 1  # 정상적으로 입력이 들어오지 않았거나 / watch, hint가 입력되었거나 / 처음 입력받을 때 flag=1
    while flag == True:  # 숫자 입력이 아닐 때 계속 입력받는 것을 반복함
        flag = False
        eq_print()
        if mouth == True:
            print("수를 입력하세요. help를 입력하시면 도움말이 출력됩니다.")
        ...
        if str_temp == 'watch':  # watch라면, 지금까지의 기록을 모두 출력함(play_list에 저장되어 있음)
            if len(play_list) == 0:  # play_list에 아무것도 없다면 출력하지 않음
                print("아직 아무것도 없습니다.")
            else:
                eq_print()
                cnt = 1
        ...
        if str_temp == 'help':#도움말 출력
            eq_print()
            print("""watch를 입력하면 지금까지 입력한 시도들이 출력됩니다.
hint를 입력하면 무언가 나오긴 합니다.
RE를 입력하시면 처음부터 다시 하실 수 있습니다.
memo를 입력하시면 메모장이 켜집니다.""")
            flag = True
        if str_temp == 'hint' and mouth == True:#힌트를 줍니다. 이 때 입력한 횟수에 따라 다릅니다.
            if hint_flag == 0:
...
def check():
    ...
    for c in str:
        if '0' <= c <= '9':  # 입력받은 str에 숫자가 있다면 cnt를 세줌
            num_cntt += 1
            if num_chkt[int(c)] == 1:  # 만약 입력한 숫자들 중 중복되는 숫자가 있다면 ifdouble을 체크함
                num_ifdoublet = 1
            else:
                num_chkt[int(c)] = 1
...
def response(cnt, ifdouble):
    ...
    if cnt != game_num:  # 원하는 자릿수와 cnt가 다르다면, 잘못된 입력임
        if mouth == True:
...
def prior_print(try_n, life):
    if if_prankster and mouth == True:  # 만약 장난을 쳤다면(입력을 이상하게 넣었다면) 경고함
        print("%d번째 몸부림" % try_n)
    else:
        print("%d번째 시도" % try_n)
