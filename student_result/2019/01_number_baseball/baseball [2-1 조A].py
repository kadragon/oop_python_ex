...
만약 Strike 와 Ball 이 하나도 없을 경우 OUT! 이 출력되며
3Strike 에 성공할 경우 게임은 당신의 승리로 끝납니다!

게임 도중 게임을 그만두고 싶다면 exit 을 입력해주세요!
...
# 사용자가 입력한 답과 정답을 비교하여 Strike 와 Ball개수 를 세는 함수
...
def ingame(n, balls):
    ...
    for i in range(n):  # 목숨의 개수만큼 입력을 받음
        ...
        attempt_number = list(map(int, attempt))

        b, s, o = match_number(attempt_number, balls)  # Strike, Ball 의 개수와 Out 여부를 판단함

        if o == 1:
            print("OUT!!")  # o==1인 경우 OUT!! 출력

        elif s == 3:
            print("3 Strike!! You Won!!")  # 3Strike 일 경우 3 Strike!! You Won!!을 출력
            k = restart()  # 게임 재시작 여부를 판단
            break  # 이후 진행된 게임이 종료된 뒤 이전 게임의 진행을 종료

        else:
            print("%d Ball  %d Strike!!" % (b, s))  # Out 이거나 3Strike 가 아닌 경우 Strike 와 Ball 의 개수를 출력
...
# 목숨의 개수를 입력받고 ingame 함수를 계속 실행하는 함수
def number_baseball():
    while True:
