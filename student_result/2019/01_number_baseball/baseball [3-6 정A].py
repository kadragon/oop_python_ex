import random
Flag = False #Flag는 Strike이 3일때 for문을 끝내기 위해 사용한다.
    ...
    try:
        for i in range(len(x)):
            if int(x[i]) in number:
                number.remove(int(x[i])) #같은 숫자가 입력되는 것을 막기 위해 list에서 제거
                tmp += 1
        if tmp == 3:
            return True
        else:
...
# Ask 함수 : 게임을 다시 할 것인지 묻는 함수. True 또는 True를 반환한다.
def Ask(ask):
    if ask == 'Y':
        return True
    if ask == 'N':
...
while True:
    number = list(range(10)) # 0부터 9까지 저장된 number라는 list를 만든다.
    random.shuffle(number) # 임의로 number의 순서를 섞는다.
    Flag = False #게임이 다시 시작됬을 때를 고려해 Flag를 False로 초기화 한다.
    UnknownNumber = ""
    for i in number[:3]:
        UnknownNumber += str(i) # 랜덤으로 섞은 number 리스트에서 앞의 값 3개를 뽑아 미지의 수를 UnknownNumber로 저장한다.
    ...
    for i in range(Chance):
        ...
        if Check(a) == True: # Check 함수를 호출하여 서로 다른 숫자 3개를 입력했는지 확인하고
            result(a)
            if Flag == True: # Strike == 3 을 만족했을 때 for문을 탈출한다.
                break
            print("="*40)
        Chance -= 1 # 서로 다른 숫자 3개를 입력한 것이 아니라면 기회를 -1 한다.

    if Flag == True: # 3자리 숫자를 맞추면 축하해주는 문구와 함께 Ask 함수를 호출하여 게임을 다시 할 것인지 묻는다.
