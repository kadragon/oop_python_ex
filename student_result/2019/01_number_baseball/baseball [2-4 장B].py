...
def makenumber():  # 각각의 자리가 0~9 중 하나로 이루어져 있는 임의의 3자리 수를 생성하는 함수
    global answer  # 전역변수 answer 의 값을 함수 안에서 수정할 수 있게 함
...
def filtering(number):  # number 은 사용자가 입력한 값으로, 이 값이 조건을 만족하는지 확인
    for j in number:  # j는 number 문자열에서 뽑아낸 하나의 값
        if number.count(j) > 1:  # number 문자열 안에 j가 몇 번 들어있는지를 알려주는 count 를 통해 그 값이 2 이상이면 같은 숫자가 중복되는 값이므로 제외
            return False
...
def refree(guess, ans):  # guess 는 사용자가 입력한 값, ans 는 정답
...
def again():  # 다시 할 것인지를 물어보는 함수
    ...
    x = input()
    if x == "yes":  # yes 를 입력하면 True 를 반환하여 반복하여 진행할 수 있게 함
        return True
    else:  # 이외에는 False 를 반환하여 게임을 끝내도록 함
        return False
