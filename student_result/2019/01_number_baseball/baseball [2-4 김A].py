...
def make_sec_num(num_len: int) -> str:
    ...
    :return: 랜덤하게 num_len 길이만큼의 숫자 문자열을 return 합니다
    """
...
def num_right(user_n: str) -> bool:
    """
    user 가 입력한 숫자문자열이 양식에 맞는지 확인합니다.
    1. 문자열이 숫자로만 구성되어있는지 확인합니다.
    2. 반복된 숫자가 없는지 확인합니다.
    :param user_n: user 가 입력한 숫자 문자열입니다
    :return: user_n이 양식에 맞다면 True, 그렇지 않으면 False 를 return 합니다
    """
    for k in user_n:
        if '0' > k or '9' < k:
...
def num_cmp(user_n: str, ans_num: str, num_len: int) -> bool:
    """
    user 가 입력한 답과 정답을 비교하여 s,b,o를 출력해주고
    정답인지 아닌지도 알려줍니다.
    또한 user 의 입력과 s,b,o 기록을 memo 에 저장합니다
    :param user_n: user 가 입력한 답입니다
    :param ans_num: 정답입니다
    ...
    for k in user_n:
        if k in ans_num:
            ...
            sbo_li[2] += 1  # 정답과 user 의 추측을 비교하여 s,b,o 점수 매김

    print("s:%d b:%d o:%d" % (sbo_li[0], sbo_li[1], sbo_li[2]))  # s,b,o를 유저에게 보여줌
    print()

    memo.append((user_n, sbo_li[0], sbo_li[1], sbo_li[2]))  # memo 에 user 의 숫자와 s,b,o를 저장함
...
def len_input() -> int:
    """
    user 가 게임을 몇자리로 플레이할지 입력받는 함수입니다
    :return: user 가 설정한 길이를 return 합니다
    """
    ...
    while type(num_len_ip) is not int:
        ...
        if k.isdigit() is False or len(k) >= 2 or k < '1':
            print("똑바로 좀 입력합시다")
            continue  # user 가 입력하는 input 이 올바른지 확인하고 그렇지 않으면 계속해서 다시 받음
        else:
            num_len_ip = int(k)
            print("좋아요 %d자리 숫자로 플레이하죠" % num_len_ip)
            print()  # user 가 제대로 입력했으면 user 의 선택을 다시 한번 알려줌

    return num_len_ip  # user 가 선택한 게임 진행 숫자의 길이 return


def get_input(num_l: int, cnt: int) -> str:
    """
    user 로 부터 num_l만큼 길이의 숫자 문자열을 입력받는 함수입니다.
    입력이 양식에 맞는지 체크하고 그렇지 않을 경우 다시 입력하도록합니다
    :param num_l: user 가 입력해야하는 숫자문자열의 길이입니다
    :param cnt: 턴 수 입니다
    :return: user 가 입력한 숫자 문자열을 return 합니다
    """
    user_n = ""
    print("%d번째 턴입니다. 숫자를 입력하세요" % cnt)
    print("참고로 숫자 대신 help 를 입력하면 숨겨진 함수를 볼 수 있습니다")
    ...
    return user_n  # user 의 입력 return


def play_again() -> bool:
    """
    다시할지 그만둘지를 입력받는 함수입니다
    :return: Yes 가 입력되면 True 를, No가 입력될 경우 False 를 return 합니다
    """
    play = ''
    ...
    while play != "Yes" and play != "No":
        ...
        if play == "Yes":
            print("좋아요 다시합시다")
            return True  # Yes 를 입력할 경우 True return

        elif play == "No":
            print("넹 잘가요ㅃㅃ")
            return False  # No 를 입력할 경우 False return

        else:
            print("Yes 나 No 중에 하나로 답하세요^^")  # Yes 나 No가 아닐 경우 다시 입력하도록 경고
...
def show() -> None:
    ...
    """
    for k in memo:
        print("num:%s | s:%d | b:%d | o:%d" % (k[0], k[1], k[2], k[3]))  # memo 에 저장된 기록을 출력해줌
    print()
