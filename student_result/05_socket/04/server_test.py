...
#필터링 문장 목록
warning_list=['I hate Python', 'Java is better than Python', 'Python is rubbish']
...
def from_client(client_socket, num):
    ...
    while True:
        ...
        if not data:
            ...
            client_socket.send(bytes("Server is going to remove your profile", 'utf-8'))
            break
    ...
    #메세지 전송이 끝나면 접속자를 제거
    print("id {}, {} has normally disconnected".format(client_id[num], client_socket))
    del client_id[num]
    client_list.remove(client_socket)
    client_socket.close()
    return 0
...
def connect():
    ...
    while True:
        ...
        #접속자의 이름을 묻는 스레드 생성
        thread_set_name=threading.Thread(target=set_name, args=(client_socket, num))
        ...
        thread_recv=threading.Thread(target=from_client, args=(client_socket, num))
        thread_recv.start()
