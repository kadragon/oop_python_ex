...
def check_probability(board, side):  # 놓았을 때 이길 수 있는 곳이 있는 지 확인
    if (board[4] == board[0] and board[0] == board[8]) or (board[4] == board[1] and board[1] == board[7]) or (
            board[4] == board[3] and board[3] == board[5]) or (board[4] == board[2] and board[2] == board[6]):
        if board[4] == side:
            return True
