    ...
    while True:
        if check == 1:
            ...
            if mode == 1:  # EASY 모드이다. 저장된 position을 그대로 넣어준다.
                ...
                if position != -1:
                    if newflag == 1:
                        standard_map[(position - 1) // 3][
                            (position - 1) % 3] = c  # flag가 0인 것은 어디든 꼭 놓아야 하는 점이 있다는 것이다. 공격이든 방어든 말이다.
                    else:
                        position = dropComputer(position, a,
                                                standard_map)  # flag가 1이면 꼭 놓아야 하는 점은 없다. 그러므로 dropComputer에 포함된 전략으로 착수한다.
