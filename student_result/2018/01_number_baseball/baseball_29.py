...
def get_random_number():  # 문자형으로 이루어진 임의의 N자리 숫자 반환
    ...
    # str(number)                 # 문자형으로 변환
    # 정말 문자형으로 변경하려고 했다면, number = str(number) 라고 해야함
    secret_number = []  # secret_number 를 리스트형으로 선언
    for i in range(N):
        secret_number.append(number.pop())  # 임의의 N자리 수를 리스트로 생성
    return secret_number  # secret_number 반환


def different(num):  # 숫자 num 중 겹치는 수의 유무
    for ic in range(N):
        for jc in range(N):
...
def judge(number):  # number(list 형)의 문자길이와 정수형 판단
    if len(number) != N:  # number 의 문자길이 판단
        return False
    if not number.isdigit():  # number 의 정수형 판단
        return False
    else:
...
def ball(host, custom):  # ball 수 찾기
    ...
    for ib in range(N):
        for jb in range(N):
            # 추측한 수와 동일한 숫자가 있을 시 count2에 +1 단, 같은 위치에 있으면 strike 임으로 같은 위치는 제외
            if ib != jb and host[ib] == int(custom[jb]):
                count2 += 1
    return count2
...
while True:
    ...
    while not judge_play(N):  # judge_play 함수 호출
        print('You have to play only 1~10')
        N = input()

    N = int(N)
    secret = get_random_number()  # get_random_number 함수 실행

    while turn <= 10:
        print('guess %d:' % turn)
        guess = input()
        while not judge(guess):  # judge 함수 호출
            print('guess again.')
            guess = input()
        if strike(secret, guess) != N:  # 모두 strike 이면 게임 종료
