...
with requests.Session() as s:
    ...
    while LOGIN_stat == 0:
        try:
            ...
            main_page = bs(
                s.get('https://go.sasa.hs.kr/main').text, 'html.parser')
            user = main_page.select(
                'li.dropdown.user.user-menu a span')[0].getText()     # 이름(학번)
            ...
            guide_list = GUIDE_APP_page.select(
                'div.col-md-9 table.table.table-bordered.table-hover tbody tr')
            ...
            if int(current_time) >= 1630:
                    ...
                    if guide_day == today:
                        if approve_stat == '대기':
                            ...
                            while re.startswith('Y') == False and re.startswith('N') == False:
                                re = input(
                                    "%s 선생님께 기숙사에 기타지도 승인 연락을 요청하는 메시지를 보낼까요?(Y/N) " % guide_teacher).strip().upper()

                                if re.startswith('Y'):
                                    APP_INFO['uidT'] = guide_teacher
                                    SMS_page = bs(
                                        s.get('https://go.sasa.hs.kr/sms/main').text, 'html.parser')
                                    csrf = SMS_page.find(
                                    ...
                                    teacher_json = s.get(
                                        "https://go.sasa.hs.kr/autocomplete/get_smsList?term=" + APP_INFO['uidT'])
                                    teacher_ID = teacher_json.text.split(
                                        ',')[1].split('"')[3].split('_')[0]
                                    MESSAGE_INFO['recvList'] = teacher_ID + \
                                        '_' + APP_INFO['uidT']

                                    MESSAGE_INFO['message'] = WAIT_message
                                    sms_req = s.post(
                                        'https://go.sasa.hs.kr/sms/main', data=MESSAGE_INFO)
                ...
                while True:
                    ...
                    while rep.startswith('Y') == False and rep.startswith('N') == False:
                        ...
                        if rep.startswith('Y'):
                            ...
                            while res.startswith('Y') == False and res.startswith('N') == False:
                                res = input(
                                    "기타지도 신청에 대한 설명을 보겠습니까?(Y/N) ").strip().upper()
                                ...
                                if res.startswith('Y'):
                                    ...
                                    print("       이외시간 -> 5")
                                    print(
                                        "             지도 시간 입력 (예): 13:00~14:00")
                                    print(
                                        "단, '이외시간'을 신청하면, 지도 시간 입력(형식 준수!!!)을 해야한다.")
                                    print('"동료" (예): 홍길동, 홍길순')
                            ...
                            if APP_INFO['nightTime'] == '5':
                                APP_INFO.update(
                                    {'nightTime2': input("지도 시간 입력: ")})
                            APP_INFO['uidT'] = input("지도교사: ")
                            APP_INFO['nightReason'] = input("사유: ")
                            APP_INFO['nightPlace'] = input("장소: ")
                            APP_INFO['with_student'] = input("동료: ")

                            friends_list = APP_INFO['with_student'].replace(
                                ' ', '').split(',')
                            APP_INFO['with_student'] = ''

                            for i in friends_list:
                                friend_json = s.get(
                                    "https://go.sasa.hs.kr/autocomplete/get_smsList?term=" + i)
                                friend_ID = friend_json.text.split(',')[1].split('"')[
                                    3].split('_')[0]
                                APP_INFO['with_student'] += friend_ID + \
                                    '_' + i + ', '

                            GUIDE_APP_page = bs(
                                s.get('https://go.sasa.hs.kr/night_Control/user').text, 'html.parser')
                            csrf = GUIDE_APP_page.find(
                                'input', {'name': 'csrf_test_name'})
                            APP_INFO.update({'csrf_test_name': csrf['value']})
                            app_req = s.post(
                                'https://go.sasa.hs.kr/night_Control/user', data=APP_INFO)
                            ...
                            alert = alert_page.select(
                                'body div div.content-wrapper script')[0].getText().strip()
                            ALARM = "alert('"+student_ID+'_' + \
                                student_name+"는 중복 신청으로 제외 되었습니다.');"

                            if alert == ALARM:
                                print("*** %s_%s는 중복 신청으로 제외 되었습니다. ***" %
                                      (student_ID, student_name))
                            else:
                                print("-"*70)
                                re = ''
                                while re.startswith('Y') == False and re.startswith('N') == False:
                                    re = input("%s 선생님께 승인 요청 메시지를 보낼까요?(Y/N) " %
                                               APP_INFO['uidT']).strip().upper()
                    ...
                    GUIDE_APP_page = bs(
                        s.get('https://go.sasa.hs.kr/night_Control/user').text, 'html.parser')
                    guide_list = GUIDE_APP_page.select(
                        'div.col-md-9 div.box-body tbody tr')
