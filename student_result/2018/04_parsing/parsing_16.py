...
first_album_url = "https://www.melon.com/album/detail.htm?albumId=" + \
    get_first_albums_number(soup)  # 1등 앨범의 url
...
def find_youtube_url(song_name):  # 멜론의 1등 앨범의 url 링크를 얻어옴
    ...
    while 'google' in song_list_href or 'channel' in song_list_href:  # 예외 처리 : 광고나 채널이 맨 위에 나타날 경우
        idx += 2
        song_list_href = you_soup.select(
            'div.yt-lockup-content a')[idx].get('href')
    return "https://www.youtube.com" + song_list_href


def get_album_info(soup_prime):  # 앨범의 정보를 얻어옴
    song_name = soup_prime.select('div.entry div.song_name')[
        0].getText().split()[1]  # 앨범 이름
    artist = soup_prime.select('div.entry div.artist a')[
        0].getText().strip()  # 앨범 아티스트
