...
with requests.Session() as s:
    ...
    check = 100
    for j in board3:
        if(j.getText() == '벌점' or j.getText() == '상점'):
            check = 3
        check -= 1
        if(check == 0):
            list1.append(j.getText())
        if(len(j.getText()) > 10):
            list1.append(j.getText().strip())
    ...
    if(int(list1[-2]) < -7 and int(list1[-2]) > -10):
        print("벌점 위험, 빨리 봉사를 하세요.", list1[-2])
    # 벌점이 10점 이상이면 이미 손쓸 수 없게 되었다고 출력한다.
    elif(int(list1[-2]) <= -10):
        print("이미 손쓸 수 없게 되었어요.", list1[-2])
    ...
    for i in range(5):
        # 문자열 합으로 주소를 넣어준다.
        section_board_list_data = bs(
            s.get('https://go.sasa.hs.kr/board/lists/2/page/'+page[i]).text, 'html.parser')
        board_list_data = section_board_list_data.select('table.table tr a')
        for j in board_list_data:
            # 일단 글의 내용을 긁어온다.
            board2 = bs(s.get('https://go.sasa.hs.kr' +
                              j.get('href')).text, 'html.parser')
