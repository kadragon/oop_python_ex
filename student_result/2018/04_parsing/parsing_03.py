...
while True:
    ...
    with requests.Session() as s:  # 로그인하는 Session
        ...
        for i in song_good:
            # 각 노래의 좋아요 숫자를 현재 순위별로 저장
            song_good_list.append(i.getText().strip())
        song_name = song.select('td')  # td 를 모두 추출
        song_is_good = song.select(
            'td.text-center button')  # 노래의 좋아요/취소 상태를 추출
            ...
            try:
                for i in song_name[2 + 6 * cnt]:  # td 안에서 노래 이름이 있는 위치에서 노래이름을 추출
                    song_name_list.append(
                        song_name[2 + 6 * cnt].getText().strip())
            except IndexError:
                break
        for i in range(len(song_good_list)):  # 노래이름을 현재 순위대로 저장
            print('{}'.format(i + 1), end=' ')
            print(song_is_good_list[i], song_good_list[i],
                  ': ', song_name_list[i * 3])
        ...
        for i in button_info:
            ...
            if t == None:  # 맨 첫번째줄 정보 버림
                continue
            # 좋아요의 경우 앞의 4글자를 버림
            button_like_id.append(t[4] + t[5] + t[6] + t[7])
            if t[0] == 'l':
                button_unlike_id.append(t[0])  # indexerror 방지
            elif t[0] == 'u':
                button_unlike_id.append(
                    t[6] + t[7] + t[8] + t[9])  # 싫어요의 경우 앞의 6글자를 버림
        ...
        if check == '1':  # 좋아요를 누르는 경우
            ...
            try:  # 좋아요를 누르는 Session
                first_page_1 = s.get(
                    'https://go.sasa.hs.kr/RcmndMusic/musicView')
                html = first_page_1.text
                soup = bs4.BeautifulSoup(html, 'html.parser')
                dict_1 = {'musicIdx': button_like_id[like], 'action': 'insert'}
                like_req = s.post(
                    'https://go.sasa.hs.kr/RcmndMusic/likeControl', data=dict_1)
            ...
            try:  # 취소를 누르는 Session
                ...
                soup = bs4.BeautifulSoup(html, 'html.parser')
                dict_1 = {
                    'musicIdx': button_unlike_id[unlike], 'action': 'delete'}
                unlike_req = s.post(
                    'https://go.sasa.hs.kr/RcmndMusic/likeControl', data=dict_1)
