...
def get_infor(i):
    ...
    video_date = i.find('span', {'class': 'time'}
                        ).getText().replace('재생시간', '')  # 영상 재생시간
    video_play = int(i.find('span', {'class': 'play'}).getText().replace(
        '재생 ', '').replace(',', ''))  # 영상 재생횟수
...
for i in range(len(infor_list)):
    print('[%s] %s(%s) | 조회수 : %s |' % (infor_list[i][0], infor_list[i]
                                        [1], infor_list[i][2], infor_list[i][3]), end=' ')
